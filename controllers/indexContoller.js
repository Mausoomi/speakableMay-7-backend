...
exports.Update_Teacher = async (req, res, next) => {
  try {
    ...
    res.json({ message: "Teacher data updated successfully", updatedTeacher });
  } catch (error) {
...
exports.SearchPaymentbyStudentUsername = async (req, res) => {
  try {
    ...
    const foundedPayment = await Payment.findOne({
      Student_ID: searchedStudent._id,
    }).populate([
      {
        path: "Package_ID",
...
exports.authorizePayment = catchAsyncErrors(async (req, res) => {
  try {
    ...
    await createBookingAndPayment(orderData, invoiceFileName);
...
async function generateInvoiceAndSave(orderData) {
    ...
    try {
      for (const val of orderData.data.orders) {
        if (val.buyer) {
          ...
          const response = await axios.post('https://invoice-generator.com', requestBody, config);
  ...
  async function createBookingAndPayment(orderData, invoiceFileName) {
    if (orderData.data) {
      for (const val of orderData.data.orders) {
        if (val.buyer) {
          const buyerUsername = val.buyer.firstName;
  ...
  async function notifyUsersAndUpdatePackage(orderData) {
    ...
    console.log(val);
            ...
            const product_id = val.products[0].name;
            const unitcost = val.products[0].unitPrice;
            const package = await Package.findById(product_id).populate('Teacher_IDs');
...
// exports.FindInvoice = catchAsyncErrors(async (req,res) => {
//   try {
//     console.log(req.body)
//     const invoicePath = re.params.invoiceName
